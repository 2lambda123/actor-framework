...
class init_fun_factory<Base, stateful_t<State>> {
public:
  static_assert(std::is_base_of_v<local_actor, Base>,
                "Base does not extend local_actor");
  ...
  template <class... Ts>
  fun operator()(stateful_t<State> f, Ts&&... xs) {
    using impl_t = typename stateful_t<State>::impl_type;
    if constexpr (sizeof...(Ts) > 0) {
      return fun{
        [f, args = std::make_tuple(detail::spawn_fwd<Ts>(xs)...)] //
