...
uuid operator"" _uuid(const char* cstr, size_t cstr_len) {
  if (cstr_len != 36)
    test::runnable::current().fail("malformed test input");
...
TEST("UUIDs print in 4 2 2 2 6 format") {
...
TEST("make_uuid parses strings in 4 2 2 2 6 format") {
...
TEST("version 1 defines UUIDs that are based on time") {
